...
def test_extract_target_files_normalizes_and_deduplicates(context):
    ...
    result = context.extract_target_files(
        "Fix ./app\\core\\parser.py and app/core/parser.py; inspect tools/helper.py."
    )
...
def test_find_related_files_scores_same_package_and_dependency_relationships(
    ...
    memory = FakeProjectMemory(files=files)
...
@pytest.mark.unit
def test_determine_strategy_selects_expected_strategy_types(context):
...
@pytest.mark.unit
def test_determine_strategy_uses_architecture_aware_feature_when_related_files_exist(
    context,
):
    ...
    assert result["strategy"]["related_file_count"] >= 1


@pytest.mark.unit
def test_multiple_targets_use_multi_target_fix_strategy(context):
    ...
    result = context.build(
        "Fix app/core/parser.py and app/core/tokenizer.py"
    )
...
@pytest.mark.unit
def test_to_prompt_returns_valid_json(context):
