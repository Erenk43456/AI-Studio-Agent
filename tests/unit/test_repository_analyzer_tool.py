...
@pytest.mark.unit
def test_repository_analyzer_execute_with_project_memory_updates_memory(
    tmp_path,
):
    (tmp_path / "main.py").write_text(
        "def main():\n    pass\n",
    ...
    class FakeProjectMemory:
        def __init__(self):
            self.project_info_calls = []
            self.architecture_calls = []
            self.file_calls = []

        def update_project_info(self, data):
            self.project_info_calls.append(data)
        ...
        def update_architecture(
            ...
            self.architecture_calls.append(
                {
                    "name": name,
                    "data": data,
                }
    ...
    assert (
        len(
            project_memory.project_info_calls
        )
        == 1
    )

    assert (
        project_memory.project_info_calls[0][
            "repository_analyzed"
        ]
        is True
    )
    ...
    assert (
        len(
            project_memory.architecture_calls
        )
        == 1
    )

    assert (
        project_memory.architecture_calls[0][
            "name"
        ]
        == "repository_analysis"
    )